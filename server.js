  ...
  try {
    ...
    if (fileName === 'customer') {
      ...
      return res.json(rows.map(c => ({
        ...c,
        orderHistory: typeof c.orderHistory === 'string'
          ? JSON.parse(c.orderHistory)
          : (c.orderHistory || [])
      })));
    }
    ...
    if (fileName === 'admin_activity') {
      const [rows] = await pool.execute(
        'SELECT * FROM admin_activity ORDER BY timestamp DESC'
      );
      return res.json(rows);
    }
    ...
    if (fileName === 'admin_notifications') {
      const [rows] = await pool.execute(
        'SELECT * FROM admin_notifications ORDER BY timestamp DESC'
      );
      return res.json(rows);
    }
    ...
    if (fileName === 'cart') {
      const [rows] = await pool.execute(
        `SELECT c.productId AS id, c.quantity,
                p.name, p.category, p.price, p.image,
                p.description, p.rating, p.stock
         FROM cart c
    ...
    if (fileName === 'wishlist') {
      const [rows] = await pool.execute(
        'SELECT productId FROM wishlist WHERE userId = ?',
        ['global']
      );
      return res.json(rows.map(r => r.productId));
    }
...
app.post('/api/:file', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: 'Unknown API target' });
  ...
  try {
    await pool.execute(
      `INSERT INTO products
       (id, name, category, price, stock, expiryDate,
        image, description, addedBy, addedAt, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
       name=VALUES(name),
